...
public class HttpUtils {
        ...
        try (DataOutputStream out = new DataOutputStream(conn.getOutputStream())){
            ...
            if (filePathMap != null && !filePathMap.isEmpty()) {
                for (Entry<String, byte[]> entry : filePathMap.entrySet()) {
                    HttpUtils.writeFile(entry.getKey(), entry.getValue(), boundary, out);
                }
            }
    ...
    private static void writeSimpleFormField(String boundary, DataOutputStream out, Entry<String, Object> entry) throws IOException {
        String boundaryStr = BOUNDARY_PREFIX + boundary + LINE_END;
        out.write(boundaryStr.getBytes());
